...
class ScreenshotSorterApp:
    ...
    def sort_and_archive_in_source(self, source_folder):
        ...
        date_folder_name = "-".join(date_part.split("-")[::-1])
        ...
        for file in files:
            ...
            try:
                location, character = self.extract_info(file)
                if not location or not character:
                    continue

                player_folder = os.path.join(main_date_folder, location, character)
    ...
    def copy_to_destination(self, dest_folder, source_folder):
        ...
        for file in files:
            ...
            player_folder = os.path.join(dest_folder, date_folder_name, location, character)
            os.makedirs(player_folder, exist_ok=True)

            dest_file_path = os.path.join(player_folder, file)
            ...
            if os.path.exists(dest_file_path):
                ...
                while True:
                    ...
                    dest_file_path = os.path.join(player_folder, new_name)
